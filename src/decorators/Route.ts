...
export function Route(options: RouteOptions): RouteDecorator;
export function Route(method: string, path: string): RouteDecorator;
export function Route(optionsOrMethod: string | RouteOptions, path?: string): RouteDecorator {
  return function (
    target: object,
    key: string | symbol,
...
export function Get(pathOrOptions: string | RouteOptions) {
  return Route({
    ...(typeof pathOrOptions === 'string' ? {path: pathOrOptions} : pathOrOptions),
    method: 'GET',
  });
}
export function Post(pathOrOptions: string | RouteOptions) {
  return Route({
    ...(typeof pathOrOptions === 'string' ? {path: pathOrOptions} : pathOrOptions),
    method: 'POST',
  });
